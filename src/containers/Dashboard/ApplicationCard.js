...
const ApplicationCard = ({ job, setApplicationModal, setJob, updateJob }) => {
  ...
  const handleStatusChange = e => {
    e.stopPropagation();
    updateJob({ ...localJob, status: e.target.value });
    setLocalJob({ ...localJob, status: e.target.value });
  };
              ...
              >
                <div className="mr-1 mt-2 ml-1">
                  <MaterialIcon
                    ...
                    color={getColor(localJob.status)}
                    size={18}
                  />
                </div>
                <select
          ...
          <div>
            ...
            <div className="mt-4 float-right pr-4  flex">
              ...
              <div className="cursor-pointer mx-2 hover:text-lg" >
                <MaterialIcon icon="delete" onClick={(e)=>handleDelete(e)}/>
              </div>
              {localJob.link ? (
                <div className="cursor-pointer mx-2 hover:opacity-50">
                  <a href={localJob.link} target="_blank" onClick={(e)=>e.stopPropagation()}>
